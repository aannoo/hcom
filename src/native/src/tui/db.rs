...
fn load_recently_stopped(conn: &Connection, now: f64) -> Vec<Agent> {
    load_stopped(conn, now, Some(600.0))
}
...
fn load_orphans(conn: &Connection) -> Vec<OrphanProcess> {
    ...
    for (pid_str, info) in &pidmap {
        ...
        // Skip if PID belongs to an active instance
        if active_db_pids.contains(&pid) {
            continue;
        }
...
fn parse_status_or_life_row(
    ...
    let (kind, tool_name, detail_text) = if status == "active" && context.starts_with("tool:") {
...
fn check_relay_enabled() -> bool {
    (|| -> Option<bool> {
        ...
        // Default true when absent (matches Python's `relay_enabled: bool = True`)
        Some(
            relay
                .get("enabled")
