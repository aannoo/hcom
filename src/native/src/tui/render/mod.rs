...
fn render_inline(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    // Layout: topline + status + sep + agents + [launch] + input(3) + footer
    let chrome = 1 + 1 + 1 + 3 + 1 + launch_height;
...
fn render_vertical(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    outer.push(Constraint::Length(3)); // input
...
fn render_input(frame: &mut Frame, area: Rect, app: &App) {
    ...
    if app.ui.mode == InputMode::Compose && !app.ui.input.is_empty() {
        ...
        let prefix_w = 2 + UnicodeWidthStr::width(input_prefix_str(app));
        let input_w = prefix_w + UnicodeWidthStr::width(app.ui.input.as_str());
        if input_w + hint_w + 2 <= mid.width as usize {
            frame.render_widget(
