...
impl App {
    pub fn handle_key(&mut self, code: KeyCode, modifiers: KeyModifiers) {
        ...
        if code == KeyCode::Char('o') && ctrl {
            if self.ui.mode == InputMode::Launch {
    ...
    fn handle_compose(&mut self, code: KeyCode) {
        match code {
            ...
            KeyCode::Tab => {
                // Enter launch from compose — preserve input (becomes launch prompt)
                self.ui.mode = InputMode::Launch;
                self.ui.launch.options_cursor = None;
                self.ui.msg_scroll = 0;
            }
            KeyCode::Backspace => {
                if self.ui.input.is_empty() {
                    // Backspace on empty compose → back to Navigate
                    self.ui.mode = InputMode::Navigate;
                } else {
    ...
    fn handle_launch_inline(&mut self, code: KeyCode) {
        ...
        match code {
            ...
            KeyCode::Enter => {
                ...
                let terminal = &self.ui.launch.terminal_presets[self.ui.launch.terminal];
